...
def train_tft_model(model, train_loader, val_loader, num_epochs, learning_rate, device, max_norm=1.0, weight_decay=1e-2):
    model.to(device)
...
if __name__ == "__main__":
    ...
    train_data = pd.read_csv('./data/price_training_data_processed.csv')
    val_data = pd.read_csv('./data/validation_data_processed.csv')
    input_cols = ['price', 'demand', 'demand_total', 'temp_air', 'temp_min', 'temp_max', 'dni', 'ghi']
    target_col = 'price'
    sequence_length = 240
    horizon = 10
    ...
    val_loader = DataLoader(val_dataset, batch_size=params["batch_size"], num_workers=num_workers)
    model, _ = train_tft_model(model, train_loader, val_loader, params["num_epochs"], params["learning_rate"], device, weight_decay=params["weight_decay"])
